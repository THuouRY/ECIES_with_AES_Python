...
# définir les deux fonctions pad et unpad pour le rembourrage
BS = 32
pad = lambda s: s + (BS - len(s) % BS) * chr(BS - len(s) % BS)
unpad = lambda s : s[:-ord(s[len(s)-1:])]


# la fonction encrypt en utilisant le clé commun et le mode CBC de AES
def encrypt( key, raw ):
        raw = pad(raw)
...
def decrypt( key, enc ):
        ...
        iv = enc[:16]
        cipher = AES.new(key, AES.MODE_CBC, iv )
        return unpad(cipher.decrypt( enc[16:] )) # decpder le résultat (bytes) de la base 64




#Générateur de notre courbe curve.P256
G = point.Point(curve.P256.gx, curve.P256.gy, curve=curve.P256) 

#Générer la clé privé de bob
prk = keys.gen_private_key(curve.P256)

# Rappelons que la clé public dans l'ECIES est un point dans la courbe 
pbk=prk*G
pbk_searalized=pickle.dumps(pbk) # Envoiler le entant que objet d'une classe
...
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
    ...
    while True:
        ...
        for sock in read_sockets:
            if sock == s: # Si c'est le socket de client
                data=b""
                data+=s.recv(2048)
            ...
            else: # si s'est le stdin (clavier)
